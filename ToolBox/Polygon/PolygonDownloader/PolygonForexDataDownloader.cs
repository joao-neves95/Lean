...
namespace QuantConnect.ToolBox.Polygon.PolygonDownloader
{
    ...
    public class PolygonForexDataDownloader : IDataDownloader
    {
        public PolygonForexDataDownloader(string apiKey)
        {
            this.ApiKey = apiKey;
        ...
        public IEnumerable<BaseData> Get(Symbol symbol, Resolution resolution, DateTime startDate, DateTime endDate)
        {
            ...
            switch (resolution)
            {
                case Resolution.Tick:
                    return this.PolygonAPI.GetForexHistoricTradesAsync("", "", startDate, endDate).GetAwaiter().GetResult();
